...
def main():
    ...
    # 获取最新的48小时数据作为输入
    latest_data = X_test[-1]  # 使用测试集的最后一个序列
    ...
    # 生成预测
    predictions = predict_future(model, latest_data, device, data_processor)
    ...
    time_index = [current_time + timedelta(hours=i) for i in range(24)]
    ...
    # 保存预测结果
    output_file = 'predictions_24h.csv'
